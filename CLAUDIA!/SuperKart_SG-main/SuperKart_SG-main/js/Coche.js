class Coche {
  ...
  createCoche(scene) {
    ...
    this.coche._physijs.friction_slip = 1000;
      
      this.coche.mesh.position.x =170;
      this.coche.mesh.position.z =+80;
      this.coche.mesh.rotation.y =-Math.PI;

      this.aux = new Physijs.BoxMesh(cabinaVentanaLateral, new THREE.MeshLambertMaterial({ color: 0x666666 }))
      this.aux.position.y = 1.1;
      this.aux.position.z = -0.6;
      this.aux.position.x = 0.92;  

      this.coche.mesh.add(this.aux);

      this.coche.mesh.matrixWorld.matrixWorldNeedsUpdate = true;
